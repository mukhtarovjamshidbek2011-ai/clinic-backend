...
async function replyWithReturnButton(ctx, user, sessionId = null) {
  ...
  if (returnButton.url) {
    return await ctx.replyWithMarkdown(
      '🔐 *Zam-Zam Health — Xavfsiz Kirish*\n\nTizimga sayt orqali kirish uchun quyidagi tugmani bosing:',
      {
        reply_markup: {
          inline_keyboard: [[{ text: returnButton.text, url: returnButton.url }]],
        },
      }
    )
  }
  ...
  const callbackHint = returnButton.callbackSource
    ...
    : 'Iltimos, `TELEGRAM_AUTH_RETURN_URL` yoki `TELEGRAM_CALLBACK_URL` ni HTTPS manzilga o‘rnating.'

  // If callback URL is not public (for example running on localhost), inform the user
  // that automatic site login requires a public domain. The token is still generated
  // server-side and the session flow works via session polling.
  return await ctx.replyWithMarkdown(
